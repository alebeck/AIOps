...
assert len(sys.argv) == 3, 'Please provide the origin and destination path as arguments.'
...
for _id in ids:
    ...
    df_upsampled.loc[df_upsampled['KPI ID'] == _id, 'value'] = (df_tmp['value'] - df_tmp['value'].mean()) / df_tmp[
        'value'].std()
    df_upsampled.loc[df_upsampled['KPI ID'] == _id, 'value_diff'] = (df_tmp['value_diff'] - df_tmp[
        'value_diff'].mean()) / df_tmp['value_diff'].std()
